...
function validate(form) {
  ...
  else if (!PHONE_RE.test(form.phone)) e.phone = 'Invalid number'
  ...
  return (
    <div className="min-h-screen bg-bg flex items-start justify-center px-6 py-8 font-body">
      <div className="w-full max-w-[520px] bg-surface border border-border-subtle rounded-card-lg px-8 py-8">
        <div className="mb-8">
          <span className="inline-block bg-brand text-black text-[11px] font-extrabold tracking-[0.18em] uppercase px-[10px] py-1 rounded-tag mb-[10px]">Fleet</span>
          <h1 className="font-display text-[48px] text-text tracking-[2px] leading-none">Delivery Agent</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <Field label="Vehicle Type" error={errors.vehicleType}>
            <select name="vehicleType" value={form.vehicleType} onChange={handleChange}
              style={{ color: form.vehicleType ? '#FFFFFF' : '#444444' }}
              className="w-full px-3 py-2 bg-[#0F0F0F] border border-border rounded-input text-sm font-body box-border focus:outline-none focus:border-brand transition-colors appearance-none">
              <option value="">Select (optional)</option>
              {VEHICLES.map(v => (
                <option key={v} value={v} style={{ color: '#FFFFFF' }}>
